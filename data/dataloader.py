...
def m():
    ...
    print(f'Current memory: {mem_info.rss / 1024 / 1024 / 1024:.4f} GB')
...
class PairedSCDataset(Dataset):
    def __init__(self, 
                ...
                hvg_list = None,    # names of highly variable genes
    ):
        ...
        if hvg_list is not None:
            self.rna_raw['hvg_ids'] = np.array(rna_vocab(hvg_list))
            self.log(f"Number of hvg genes: {len(self.rna_raw['hvg_ids'])}")
    ...
    def get_full_genes_ids(self):
        return self.rna_raw['gene_ids']
            
    def prepare_row_rna_hvg(self, row):
        if not isinstance(row, np.ndarray):
            row = row.toarray().flatten()
        genes = self.rna_raw['hvg_ids']
        # find the idx of hvg genes, each item in genes is a gene id
        # self.rna_raw['gene_ids'] is a numpy array of gene ids
        # for each item in genes, find the index in self.rna_raw['gene_ids']
        hvg_idx = np.array([np.where(self.rna_raw['gene_ids'] == gene)[0][0] for gene in genes])
    
        assert np.array_equal(self.rna_raw['gene_ids'][hvg_idx], genes)
        
        values = row[hvg_idx]
        chrs = self.rna_raw['chr_ids'][hvg_idx]
    ...
    def __getitem__(self, idx):
        ...
        elif self.rna_raw.get('hvg_ids') is not None:
            rna_ids, rna_values, rna_chrs = self.prepare_row_rna_hvg(rna_row)
        else:
...
# def main():
#     vocab = GeneVocab.from_file('/home/jwu418/workspace/data/ours/vocab/bmmc_rna_binned.json')
#     max_len = 36601 + 1
#     adata = AnnData.read('/home/jwu418/workspace/data/ours/test/bmmc_rna_binned.h5ad')
#     dataset = create_scDataset(adata, vocab, max_len)
#     DataLoader(dataset, batch_size=1, shuffle=True, drop_last=False, num_workers=5)
#     # try to get a batch
#     for i, batch in enumerate(DataLoader(dataset, batch_size=32, shuffle=True, drop_last=False, num_workers=5)):
#         print(batch)
#         if i == 0:
#             break
